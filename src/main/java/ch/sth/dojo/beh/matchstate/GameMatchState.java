...
public record GameMatchState(CMatch nextMatch, Satz nextSatz, Game nextGame) implements MatchState {

    private static Function3<Either<DomainProblem, CMatch>, Either<DomainProblem, Satz>, Either<DomainProblem, Game>, Either<DomainProblem, Tuple3<CMatch, Satz, Game>>> tuple3EithersToEitherTuple3 =
        (eith1, eith2, eith3) ->
            eith1.flatMap(match -> eith2.flatMap(satz -> eith3.map(game -> Tuple.of(match, satz, game))));

    static ch.sth.dojo.beh.matchstate.GameMatchState untuple(Tuple3<CMatch, Satz, Game> tuple3) {
        return tuple3.apply(ch.sth.dojo.beh.matchstate.GameMatchState::new);
    }

    MatchState apply(Function3<CMatch, Satz, Game, MatchState> applicative) {
    ...
    public Tuple3<CMatch, Satz, Game> tupled() {
        return Tuple.of(nextMatch, nextSatz, nextGame);
    }
}
