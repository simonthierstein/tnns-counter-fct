...
public record GameMatchState(Match nextMatch, Satz nextSatz, Game nextGame) implements MatchState {

    private static Function3<Either<DomainProblem, Match>, Either<DomainProblem, Satz>, Either<DomainProblem, Game>, Either<DomainProblem, Tuple3<Match, Satz, Game>>> tuple3EithersToEitherTuple3 =
        (eith1, eith2, eith3) ->
            eith1.flatMap(match -> eith2.flatMap(satz -> eith3.map(game -> Tuple.of(match, satz, game))));

    static ch.sth.dojo.beh.matchstate.GameMatchState untuple(Tuple3<Match, Satz, Game> tuple3) {
        return tuple3.apply(ch.sth.dojo.beh.matchstate.GameMatchState::new);
    }

    MatchState apply(Function3<Match, Satz, Game, MatchState> applicative) {
    ...
    public Either<DomainProblem, ch.sth.dojo.beh.matchstate.GameMatchState> apply(
        Function<Match, Either<DomainProblem, Match>> matchFunction,
    ...
    public Tuple3<Match, Satz, Game> tupled() {
        return Tuple.of(nextMatch, nextSatz, nextGame);
    }
}
